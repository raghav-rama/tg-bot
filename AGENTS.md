...
- `tests/conftest.py` `build_settings` hides every environment variable matching a `Settings` alias while it constructs test settings. The repo `.envrc` feeds `.env` into the shell through direnv, and `_env_file=None` blocks only the file, not `os.environ`, so without this a developer's real provider settings silently override what a test passes.
