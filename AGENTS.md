...
- Phase 2 code now exists for `/image <prompt>`, queued Gemini image generation through the Python `google-genai` SDK path, Telegram photo delivery, and SQLite persistence of generated-image metadata.
- Phase 2 image generation now uses Gemini Developer API key authentication for the active Google media path.
- The `/image` provider now uses current Gemini image generation models for both prompt-to-image and reference-image flows.
- Phase 3 code now exists for `/video <prompt>`, SQLite-backed generation jobs, an in-process polling worker, Gemini Interactions API video generation through the Python `google-genai` SDK path, and Telegram video delivery.
- `/image` and `/video` now share the same queued-job UX: the Telegram request path validates input, persists a job, sends an immediate acknowledgement, and the background worker performs provider submission, polling, reference-photo download, and final delivery later.
- Phase 4 video routing now supports provider fallback: `/video <prompt>` uses Gemini first and falls back according to configured provider order only on classified safety rejections.
...
- Video generation jobs now have a bounded lifetime: a video job older than `VIDEO_JOB_MAX_AGE_SECONDS` (default `1800`) is marked failed with a `video_job_abandoned` log event and a user-facing retry message, instead of being re-polled forever when the provider returns a permanent error. A job that has already been submitted is abandoned only after a poll fails to reach a terminal state, so a completed video is still delivered no matter how late the worker reaches it; a job still awaiting submission is dropped without spending a provider call. Job `created_at` is now the submission time rather than the inbound Telegram message timestamp, so a backlogged webhook delivery cannot create an already-stale job.
...
- Phase 4 now includes log-only observability for chat, `/image`, `/video`, and video job delivery, with optional config-driven cost estimates that default to disabled.
...
- Phase 4 now supports reference-image generation from Telegram photo captions and text commands that reply to a Telegram photo: `/image <prompt>` and `/video <prompt>` both support reference media inputs.
- Queued reference-photo jobs persist Telegram photo metadata plus the reusable `file_id` in SQLite job payloads; the worker re-downloads the photo later instead of persisting raw reference bytes.
...
- CI runs in GitHub Actions through `.github/workflows/ci.yml`: `uv sync --locked --extra dev` on Python 3.13 followed by `pytest`, on pushes to `main` or `dev` and on pull requests targeting them. It needs no secrets because the suite is hermetic.
- The test suite is isolated from the developer environment: an autouse `tests/conftest.py` fixture clears every environment variable matching a `Settings` alias and unsets the `.env` file for each test, and `build_settings` hides them again while it constructs test settings. The repo `.envrc` feeds `.env` into the shell through direnv, and `_env_file=None` blocks only the file, not `os.environ`, so without this a developer's real provider settings silently override what a test passes and the suite only passes on a machine that has them. Domain helpers in `app/domain/preferences.py` still fall back to a bare `Settings()` when no settings are passed; callers must pass `settings=` or the isolated tests fail loudly.
- Automated tests exist under `tests/` for health/readiness behavior, Telegram normalization, reply-to-photo reference commands, allowlist handling, history reuse, reset semantics, draft streaming, draft fallback, explicit draft rate-limit fallback, provider cleanup, supersession, image generation, video job handling, settings preferences, observability/cost-estimate helpers, Gemini provider flows, Runpod provider flows, Fal provider flows, and video provider routing.
...
- Photo captions that do not start with `/image` or `/video` continue through the OpenAI image-understanding path.
...
- Live Gemini, Runpod, Fal, and Telegram verification for the `/image` and `/video` flows still depends on real environment variables and a manual runtime check.
