...
As of `2026-06-18`, the repository is no longer docs-only. Phase 1, Phase 1.5, Phase 2, and Phase 3 are accepted as complete, and the roadmap is in Phase 4 hardening and expansion.
...
- Per-user/per-chat quotas, video/image daily budget caps, prompt/reference-image moderation gates, and in-app provider-output URL retention/expiry handling are deferred out of active Phase 4 scope.
- Fal-hosted video provider support, including likely Kling 3 and Seedance 2 presets behind the existing queued `/video` path, is planned for Phase 6 and is not active Phase 4 scope.
